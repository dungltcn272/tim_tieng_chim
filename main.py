...
# Tìm kiếm tương đồng dựa trên CSDL của bạn
...
class BirdSearchApp:
    def __init__(self, root):
        ...
        self.build_button = ctk.CTkButton(
            self.root,
            text="Xây dựng CSDL từ birdsounds",
    ...
    def _build_database_thread(self):
        if os.path.exists(DATASET_CSV) and os.path.exists(MEAN_STD_CSV):
            self.loading_label.configure(text="CSDL đã tồn tại. Bạn cần xóa hoặc đổi tên dataset.csv và mean_std.csv trước khi tạo lại.")
            return
        ...
        try:
            ...
            self.root.after(0, lambda: self.progress_bar.pack_forget())
            self.loading_label.configure(text=f"Lỗi khi xây dựng CSDL: {e}")
    ...
    def toggle_play_result(self, file_path, button):
        ...
        if current_playing:
            ...
            try:
                ...
                print(f"Lỗi khi phát file {file_path}: {e}")
                self.result_files[file_path]["button"].configure(text="Play")
                self.result_files[file_path]["playing"] = False
